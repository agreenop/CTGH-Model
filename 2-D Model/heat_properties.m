...
if inlet_prop==1 %First time, properties will be calculated at inlet temperatures and pressures
    ...
    T_g_avg=T_g_in;
    P_g_avg=P_g_in;
    [~,~,~,~,~,~,~,~,~,Pr_so,~]=Material_prop(liquid,gas,tube_material,T_l_avg,T_l_avg,P_g_avg); %Estimates gas Prandtl number at outer tube surface
    [~,~,~,~,~,Pr_si,~,~,~,~,~]=Material_prop(liquid,gas,tube_material,T_l_avg,T_l_avg,P_g_avg); %Estimates liquid Prandtl number at inner tube surface
    ...
    T_s_in=T_s_in_matrix(i,j);
    [~,~,~,~,~,~,~,~,~,Pr_so,~]=Material_prop(liquid,gas,tube_material,T_l_avg,T_s_out,P_g_avg); %Estimates gas Prandtl number at outer tube surface
    [~,~,~,~,~,Pr_si,~,~,~,~,~]=Material_prop(liquid,gas,tube_material,T_l_avg,T_s_in,P_g_avg); %Estimates liquid Prandtl number at inner tube surface
end
...
if inlet_prop==1
    ...
    Nu_g=C2*C1*Re_g^m*Pr_g^(0.36)*(Pr_g/Pr_so)^(1/4);
end    
