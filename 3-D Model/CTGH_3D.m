...
k(i)=alpha_press+2*gamma_press*log(w(i));
Q(i)=2/(3*zeta(i))*k(i)*(F_c*n/F_m)^2;
R(i)=-(f_c*L_tube_avg/(4*D_in*zeta(i)))*(F_c*n/F_m)^2;
B(i)=sign(R(i)+sqrt(Q(i)^3+R(i)^2))*abs((R(i)+sqrt(Q(i)^3+R(i)^2)))^(1/3)+sign(R(i)-sqrt(Q(i)^3+R(i)^2))*abs((R(i)-sqrt(Q(i)^3+R(i)^2)))^(1/3); %Cubic root was giving the complex roots. Using sign(x)*abs(x)^(1/3) forces Matlab to find the real cubic root.
J(i)=sign(R(i)+sqrt(Q(i)^3+R(i)^2))*abs((R(i)+sqrt(Q(i)^3+R(i)^2)))^(1/3)-sign(R(i)-sqrt(Q(i)^3+R(i)^2))*abs((R(i)-sqrt(Q(i)^3+R(i)^2)))^(1/3);
w(i+1)=exp(-B(i)*x(i+1)/2)*(sin(sqrt(3)/2*J(i)*(1-x(i+1)))/sin(sqrt(3)*J(i)/2));
